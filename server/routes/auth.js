...
import { createUser, findUserByName, updateUserSettings } from '../db.js';
...
router.put('/me/settings', authenticate, (req, res) => {
  try {
    const { skin, legacyEmail } = req.body;
    updateUserSettings(req.user.id, skin || 'heirloom', legacyEmail || '');
    req.user.active_skin = skin || 'heirloom';
    req.user.legacy_email = legacyEmail || '';
    res.json(req.user);
